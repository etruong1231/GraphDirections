...
public:
    ...
    //for exception issues when adding vertices and edges
    
    bool vertexExists(int vertex) const;

    bool edgeExists(int fromVertex, int toVertex) const;

    bool DFTr(std::map<int,DigraphVertex<VertexInfo,EdgeInfo>> graph, int vertex, std::map<int,bool> &visited, std::map<int,bool> &onPath) const;
private:
    ...
    std::map<int, DigraphVertex<VertexInfo,EdgeInfo>> path;
    //store all vertice number
...
Digraph<VertexInfo, EdgeInfo>::Digraph(const Digraph& d){
    ...
    path = d.path;
    vertices_num = d.vertices_num;
}

template <typename VertexInfo, typename EdgeInfo>
Digraph<VertexInfo, EdgeInfo>::Digraph(Digraph&& d){
    
    std::swap(path, d.path);
    std::swap(vertices_num, d.vertices_num);
}
...
std::vector<std::pair<int, int>> Digraph<VertexInfo, EdgeInfo>:: edges() const{
    ...
    std::vector<std::pair<int,int>> allVertexInfo;
    
    for(auto vertex_info: path){
        //grab the edge info from that vertex
        std::list<DigraphEdge<EdgeInfo>> edge_list = vertex_info.second.edges;
        for(auto edge_info: edge_list){
            allVertexInfo.push_back(std::pair<int,int>(edge_info.fromVertex, edge_info.toVertex));
        }
    }
    return allVertexInfo;
}
...
std::vector<std::pair<int, int>> Digraph<VertexInfo, EdgeInfo>:: edges(int vertex) const{
    ...
    std::list<DigraphEdge<EdgeInfo>> edge_list = path.at(vertex).edges;
    for(auto edge_info: edge_list){
        all_edges.push_back(std::pair<int,int>(edge_info.fromVertex,edge_info.toVertex));
    }
    return all_edges;
}
...
VertexInfo Digraph<VertexInfo, EdgeInfo> :: vertexInfo(int vertex) const{
    ...
    try{
        if(!vertexExists(vertex))
            throw DigraphException("Error: Vertex does not exists in Graph!!!");
       
        vinfo = path.at(vertex).vinfo;

           
    }catch(DigraphException& de){
        std::cout << de.reason() << std::endl;
...
EdgeInfo Digraph<VertexInfo, EdgeInfo> :: edgeInfo(int fromVertex, int toVertex) const{
    ...
    try{
        if(vertexExists(fromVertex) && vertexExists(toVertex)){
            ...
            std::list<DigraphEdge<EdgeInfo>> edges_list = path.at(fromVertex).edges;
            for(auto edge_info: edges_list){
                if(edge_info.toVertex == toVertex){
                    einfo = edge_info.einfo;
                    has_edge = true;
                }
            }
            if(!has_edge)
                throw DigraphException("Error: Vertex does not have outgoing Edge!!!");

        }else{
            throw DigraphException("Error: Vertex does not Exists!!!");
        }
...
void Digraph<VertexInfo, EdgeInfo>::addVertex(int vertex, const VertexInfo& vinfo){
    ...
    try{
        ...
        if(vertexExists(vertex))
            throw DigraphException("Error: Vertex already Exists!!!!");
        DigraphVertex<VertexInfo, EdgeInfo> new_vertex{vinfo};
        path[vertex] = new_vertex;
        vertices_num.push_back(vertex);
       }
    catch(DigraphException& de){
        std:: cout << de.reason() << std::endl;
    }
...
{
    for(auto vertex_num: vertices_num){
...
void Digraph<VertexInfo, EdgeInfo>::addEdge(int fromVertex, int toVertex, const EdgeInfo& einfo){
    ...
    try{
        //make sure that both vertex exists in the graph
        if(vertexExists(fromVertex) && vertexExists(toVertex)){
            //get the lists of edges from fromVertex
            std::list<DigraphEdge<EdgeInfo>> edge_list = path[fromVertex].edges;
            //check if the theres an edge already connected to from vertex to 
            for(auto edge_info: edge_list){
                if(edgeExists(edge_info.toVertex,toVertex))
                    throw DigraphException("Error: Already has an existing edge from vertex!!!");
            }
            //create a new digraphEdge then push back to vertex list of edges
            DigraphEdge<EdgeInfo> new_edge{fromVertex,toVertex,einfo};
            path[fromVertex].edges.push_back(new_edge);
        }
        else{
            throw DigraphException("Error: A Vertex does not exists in Graph!!!");
        }
    }catch(DigraphException& de){
...
void Digraph<VertexInfo, EdgeInfo>::removeVertex(int vertex){
    ...
    try{
        if(!vertexExists(vertex))
            throw DigraphException("Error: Cannot remove a Vertex that does not Exists!!!");

        path.erase(vertex);
        vertices_num.erase(std::remove(vertices_num.begin(), vertices_num.end(), vertex), vertices_num.end());
...
void Digraph<VertexInfo, EdgeInfo>::removeEdge(int fromVertex, int toVertex){
   ...
   try{
        if(!vertexExists(fromVertex) || !vertexExists(toVertex))
            throw DigraphException("Error: trying to remove Edge from vertice that does not exists!!!");

        std::list<DigraphEdge<EdgeInfo>> &edge_list = path.at(fromVertex).edges;
        ...
        if(!hasEdge)
            throw DigraphException("Error: trying to remove a Edge that does not exists in Vertex!!!");
...
int Digraph<VertexInfo, EdgeInfo>::edgeCount(int vertex) const{
    ...
    try{
        if(!vertexExists(vertex))
            throw DigraphException("Error: Vertex Does not exists for edgeCount!!!!");
       count = path.at(vertex).edges.size();
       }
    catch(DigraphException& de){
        std::cout << de.reason() << std::endl;
    }
...
bool Digraph<VertexInfo, EdgeInfo>::isStronglyConnected() const{
    ...
    for(auto vertex: path){
    //maybe implement a map

        std::map<int,bool> visited;
        std::map<int,bool> onPath;

        is_strongly_connected = DFTr(path, vertex.first,visited, onPath);
    }
...
template <typename VertexInfo, typename EdgeInfo>
bool Digraph<VertexInfo, EdgeInfo>::DFTr(std::map<int, DigraphVertex<VertexInfo, EdgeInfo>> graph, int vertex, std::map<int,bool> &visited, std::map<int,bool> &onPath) const{

    visited[vertex] = true;
    onPath[vertex] = true;

    std::list<DigraphEdge<EdgeInfo>> edge_list = graph.at(vertex).edges;
    for(auto edge_info: edge_list){
       if(visited.count(edge_info.toVertex) == 0)
            DFTr(graph,edge_info.toVertex,visited,onPath);
       if(onPath.count(edge_info.toVertex) == 1)
            return true;
    }
...
std::map<int, int> Digraph<VertexInfo, EdgeInfo>::findShortestPaths(int startVertex, std::function<double(const EdgeInfo&)> edgeWeightFunc) const{
    ...
    //to help get the vertex info
    std::map<int, int> vertexMap;

    std::map<int,VertexPath> vertex_data;
    //for each vertex set up data info
    for(auto vertex_info:path){
        vertex_data[vertex_info.first] = VertexPath{vertex_info.first};
    }
    //set starting vertex as first
    vertexMap[0] = startVertex;

    vertex_data[startVertex].d = 0;
    ...
    while(!pq.empty()){
         ...
         int vertex_num  = vertexMap[pq.top()];

         VertexPath v = vertex_data[vertex_num];

         if(v.k == false){

            v.k = true;

            std::list<DigraphEdge<EdgeInfo>> edge_list = path.at(v.vertex).edges;
            for(auto edge_info: edge_list){

                int outgoing_edge = edge_info.toVertex;
                double length = edgeWeightFunc(edge_info.einfo);

                if(vertex_data[outgoing_edge].d > v.d + length)
                {
                    vertex_data[outgoing_edge].d = v.d + length;
                    vertex_data[outgoing_edge].path = v.vertex;
                    pq.push(v.d + length);
                    vertexMap[v.d +length] = outgoing_edge;
                }
        ...
        for(auto shortpath: vertex_data){
            shortest_path[shortpath.first] = shortpath.second.path;
        }
